...
SELECT
    month,
    total_sales,
    total_sales - LAG(total_sales, 1) OVER (ORDER BY month) AS mom_difference,
    ((total_sales - LAG(total_sales, 1) OVER (ORDER BY month)) 
        / LAG(total_sales, 1) OVER (ORDER BY month)) * 100 AS mom_growth_percentage
FROM (
    SELECT
...
WHERE MONTH(transaction_date) = 5 -- May
    AND DAYOFWEEK(transaction_date) = 2 -- Monday (1 is Sunday, 2 is Monday, ..., 7 is Saturday)
